...
export function errorBanner(): string {
  return `
${MAGENTA}███████╗██╗  ██╗██╗██╗     ██╗     ███████╗${RESET}
${MAGENTA}██╔════╝██║ ██╔╝██║██║     ██║     ██╔════╝${RESET}
${MAGENTA}███████╗█████╔╝ ██║██║     ██║     ███████╗${RESET}
...
export function successBanner(): string {
  return `
${CYAN}██████╗ ██╗      ██████╗  ██████╗ ███████╗███████╗███╗   ███╗${RESET}
${CYAN}██╔══██╗██║     ██╔════╝ ██╔═══██╗██╔════╝██╔════╝████╗  ████║${RESET}
${CYAN}██████╔╝██║     ██║  ███╗██║   ██║█████╗  █████╗  ██╔████╔██║${RESET}
...
export function progressBox(title: string, items: { name: string; status: string; agents?: string }[]): string {
  const width = 74;
...
export function installSummary(items: { path: string; agents: string; method: string }[]): string {
  const width = 74;
...
export function securityTable(items: { name: string; gen: string; socket: string; snyk: string }[]): string {
  const width = 74;
...
export function promptBox(title: string, options: string[], selected: number): string {
  const width = 74;
  const top = `${CYAN}┌─ ${title} ${'─'.repeat(width - title.length - 5)}─┐${RESET}`;
