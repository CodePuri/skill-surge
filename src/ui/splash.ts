...
export function printCandidates(payload: { task: string; cachePath: string; notes: string[]; candidates: Candidate[] }): void {
  if (payload.candidates.length === 0) {
    console.log(boxen(
      gradient(['#FF073A', '#FF6EC7'])(`\n  No strong skill candidates found for:\n  "${payload.task}"\n`) +
      '\n  Try a more descriptive task description.',
  ...
  const header = boxen(
    gradient(['#00FFFF', '#FF00FF', '#FF6EC7'])(`  Skills Found for: "${shorten(payload.task, 60)}"`) +
  ...
  for (const c of payload.candidates) {
    ...
    let badge: string;
    if (isBundled) badge = gradient(['#FF6EC7', '#00FFFF'])(' █ PRE-LOADED ');
    else if (isLocal) badge = gradient(['#7B68EE', '#00FFFF'])(' █ INSTALLED ');
    else if (c.canAutoInstall) badge = gradient(['#39FF14', '#00FFFF'])(' █ AUTO-INSTALL ');
    else badge = gradient(['#FFD700', '#FF6EC7'])(' █ RECOMMEND ');
