...
function preferCandidate(candidate: Candidate): number {
  let value = 0;
  if (candidate.sourceKind === 'bundled') value += 10;
...
function buildReason(
  ...
  const parts: string[] = [];
  if (candidate.sourceKind === 'bundled') parts.push('pre-bundled core skill');
  ...
  if (installCount > 0) parts.push(`${installCount.toLocaleString()} installs`);
  if (canAutoInstall) parts.push('auto-install eligible');
  if (parts.length === 0) parts.push('weak metadata match');
  return parts.join('; ');
}

export function scoreCandidate(candidate: Candidate, task: string, config: Config): Candidate {
  ...
  let score = 0;
  if (candidate.sourceKind === 'bundled') score = 50;
  else if (candidate.sourceKind === 'local') score = 42;
  else score = 25;
...
export function dedupeByName(candidates: Candidate[]): Candidate[] {
...
export function rerankWithExternalCommand(task: string, candidates: Candidate[]): { candidates: Candidate[]; note: string } {
